(function(){
    ...
    /*原创第二张图*/
    var yc_two=document.querySelector(".yc_middle2")
        //鼠标进入处理函数 
        yc_two.onmouseover=function(e){
            var yc_two=this;
            //获取元素的宽高
            var w=parseInt(window.getComputedStyle(yc_two)
                  .getPropertyValue("width").replace("px",""))
            var h=parseInt(window.getComputedStyle(yc_two)
            ...
            //获取top的值
            var top=parseInt(window.getComputedStyle(yc_two)
            .getPropertyValue("top").replace("px",""))
            var yc_middle2_js=document.querySelector(".yc_middle2_js")
            if ( (k*x)>=y && (h-k*x)>y ){
                yc_middle2_js.style.left=0
                yc_middle2_js.style.top=0
        ...
        yc_two.onmouseout=function(e){
            ...
            if ( (k*x)>=y && (h-k*x)>y ){
                yc_middle2_js.style.left=0
                yc_middle2_js.style.top=`${-h}px`
            }else if ( (k*x)<y && (h-k*x)<y ){
                yc_middle2_js.style.left=0
                yc_middle2_js.style.top=`${h}px`
            }else if ( (k*x)<y && (h-k*x)>y ){
                yc_middle2_js.style.left=`${-w}px`
                yc_middle2_js.style.top=0
            }else if ( (k*x)>y && (h-k*x)<y ){
                yc_middle2_js.style.left=`${w}px`
                yc_middle2_js.style.top=0
